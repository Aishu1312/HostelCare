"""
Business logic layer sitting between the Streamlit UI and FirebaseService.
...
def submit_complaint(firebase_service, room_number: str, category: str, description: str):
    """
    ...
    success, result = firebase_service.create_complaint(room_number, category, description)
    if not success:
        return False, [result]
...
def update_status(firebase_service, complaint_id: str, new_status: str):
    """Validates the target status against the canonical list, then delegates
    to FirebaseService. Returns (success, payload)."""
    if new_status not in STATUS_OPTIONS:
        return False, f"'{new_status}' is not a recognized status."

    success, result = firebase_service.update_complaint_status(complaint_id, new_status)
    if not success:
        return False, result
